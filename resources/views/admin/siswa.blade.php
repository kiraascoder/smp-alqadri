                    ...
                    <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
                        <div class="flex items-center gap-3">
                            ...
                            <div>
                                ...
                                <p class="text-gray-600">Daftar lengkap siswa sekolah</p>
                            </div>
                        </div>
                        ...
                        <div class="flex items-center gap-2 text-sm text-gray-500">
                            ...
                            <span>Total: {{ $siswas->total() }} data</span>
                        </div>
                    </div>
                </div>
                        ...
                        <tbody class="divide-y divide-gray-100">
                            @forelse ($siswas as $item)
                                <tr class="hover:bg-blue-50/50 transition-colors duration-200 group"
                                    ...
                                    data-nisn="{{ $item->nisn ?? '-' }}" data-kelas="{{ $item->kelas->nama_kelas ?? '-' }}"
                                    data-score="{{ $item->score_bk ?? '0' }}" data-email="{{ $item->user->email ?? '-' }}"
                                    data-alamat="{{ $item->alamat ?? '-' }}" data-no-hp="{{ $item->user->no_hp ?? '-' }}"
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <div class="flex items-center justify-center gap-2">
                                            ...
                                            <!-- Tombol Hapus -->
                                            <button
                                                onclick="confirmDelete({{ $item->id }}, '{{ $item->user->name ?? 'Siswa' }}')"
                    ...
                    <div class="bg-gray-50/50 px-6 py-4 border-t border-gray-200">
                        <div class="flex flex-col sm:flex-row items-center justify-between gap-4">
                            ...
                            <div class="text-sm text-gray-600">
                                Menampilkan <span class="font-semibold text-gray-900">{{ $siswas->firstItem() }}</span>
                                sampai <span class="font-semibold text-gray-900">{{ $siswas->lastItem() }}</span>
                                dari <span class="font-semibold text-gray-900">{{ $siswas->total() }}</span> hasil
                            </div>
    ...
    <div id="addModal" class="fixed inset-0 bg-black/50 backdrop-blur-sm hidden items-center justify-center z-50 p-4">
        <div class="bg-white rounded-2xl shadow-2xl max-w-2xl w-full max-h-[95vh] flex flex-col transform transition-all duration-300 scale-95 opacity-0"
                ...
                <div class="flex items-center justify-between">
                    <div class="flex items-center gap-3">
                        ...
                        <div>
                            ...
                            <p class="text-green-100">Daftarkan siswa baru ke sistem</p>
                        </div>
                    </div>
            ...
            <div class="flex-1 overflow-y-auto p-6">
                <form id="addSiswaForm" action="{{ route('admin.siswa.register') }}" method="POST" class="space-y-6">
                    @csrf
                    ...
                    <div class="space-y-4">
                        ...
                        <div>
                            ...
                            <div class="text-red-500 text-sm mt-1 hidden" id="nisn-error"></div>
                        </div>
                    </div>
                    ...
                    <div class="space-y-4">
                        <div class="flex items-center gap-3 pb-3 border-b border-gray-200">
                            ...
                            <h4 class="text-lg font-semibold text-gray-900">Informasi Akademik</h4>
                        </div>
                        ...
                        <div>
                            ...
                            <select id="kelas_id" name="kelas_id" required
                                ...
                                @if (isset($kelasList))
                                    ...
                                    @endforeach
                                @else
                                    <!-- Default options jika $kelas tidak tersedia -->
                            ...
                            <div class="text-red-500 text-sm mt-1 hidden" id="kelas_id-error"></div>
                        </div>
                    </div>
                ...
                <div class="flex items-center justify-between">
                    <div class="flex items-center gap-3">
                        ...
                        <div>
                            ...
                            <p class="text-blue-100">Informasi lengkap data siswa</p>
                        </div>
                    </div>
            ...
            <div class="p-6">
                <div class="flex items-center gap-4 mb-4">
                    ...
                    <div>
                        ...
                        <p class="text-gray-600">Apakah Anda yakin ingin menghapus data siswa ini?</p>
                    </div>
                </div>
                ...
                <div class="space-y-6">
                    ...
                    <div class="flex items-center gap-6 p-6 bg-gradient-to-r from-blue-50 to-indigo-50 rounded-xl border border-blue-200">
                        ...
                        <div>
                            ...
                            <p class="text-blue-600 font-medium">NISN: ${data.nisn}</p>
                    ...
                    <div class="p-4 rounded-lg ${score >= 80 ? 'bg-red-50 border border-red-200' : score >= 50 ? 'bg-yellow-50 border border-yellow-200' : 'bg-green-50 border border-green-200'}">
                        <div class="flex items-center gap-3">
                            ...
                            <div>
                                <p class="font-semibold ${score >= 80 ? 'text-red-800' : score >= 50 ? 'text-yellow-800' : 'text-green-800'}">
                                    ${score >= 80 ? 'Perlu Perhatian Khusus' : score >= 50 ? 'Perlu Bimbingan' : 'Kondisi Baik'}
                                </p>
                                <p class="text-sm ${score >= 80 ? 'text-red-700' : score >= 50 ? 'text-yellow-700' : 'text-green-700'}">
                                    ${score >= 80 ? 'Skor BK tinggi, diperlukan konseling intensif' : 
                                      score >= 50 ? 'Skor BK sedang, diperlukan monitoring' : 
                                      'Skor BK rendah, siswa dalam kondisi baik'}
                                </p>
                            </div>
                        </div>
                    </div>
